...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private HashMap<Integer, ArrayList<ModelMap>> modelMaps;

    public DbHelper(Context context) {
        super(context, DBNAME, null, 1);
        modelMaps = new HashMap<Integer, ArrayList<ModelMap>>();
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + PAGESTITLE + " (TABLE_ID INTEGER,NAME TEXT)");
        ...
        db.execSQL("CREATE TABLE " + GENERALTABLEPREFIX + 6 +
                " (ID INTEGER PRIMARY KEY,TITLE TEXT,HEADER_R TEXT,HEADER_L TEXT,BODY TEXT,FOOTER_R TEXT,FOOTER_L TEXT,STARED INTEGER)");
        db.execSQL("CREATE TABLE " + USERCONFIQ + " (USERID INTEGER,USERNAME TEXT,HASUSERPERMISSION INTEGER)");
        Log.e("db", "--------------- DB created -------------------");
    }
    ...
    public ArrayList<ModelMap> getModelMap(int ix) {
        ArrayList<ModelMap> list = modelMaps.get(ix);
        if (list == null) {
            list = new ArrayList<ModelMap>();
            SQLiteDatabase db = this.getWritableDatabase();
    ...
    public void insertTableNames(ArrayList<String> lastTablesName) {
